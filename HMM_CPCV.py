...
def keep_numeric_nonconstant(X: pd.DataFrame) -> pd.DataFrame:
    """Keep the numerical columns and remove constant columns (with VAR = 0)"""
...
Note: Internally, dropna will be used (VIF is only calculated on complete-case tests).）
    """
...
def vif_prune_iterative(X: pd.DataFrame, vif_thresh: float = 10.0, max_iter: int = 100):
    """
   Step 1) VIF iterative elimination:  Remove the feature with the largest VIF in each iteration until 
   all VIF values ​​are less than or equal to vif_thresh.
Returns: X_pruned, removed_list, final_vif_table
    """
...
def mi_feature_target(X: pd.DataFrame, y: np.ndarray, task: str = "regression",
    ...
    """
 Calculate the Mutual Information (MI) between each feature and the target: MI(X_i ; y)
This is used to decide which feature to keep in redundant pairs (the one more relevant to y is kept).）。
    """
